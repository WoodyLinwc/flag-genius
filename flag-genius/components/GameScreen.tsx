...
import { addWrongCode, addRightCode } from "../storage";
...
const GameScreen: React.FC<GameScreenProps> = ({ mode, language, onBack }) => {
  ...
  const handleOptionClick = (option: Country) => {
    ...
    // Auto-track results to localStorage for Study Mode
    if (correct) {
      setScore((s) => s + 1);
      addRightCode(question.target.code);
    } else {
      addWrongCode(question.target.code);
    }
  ...
  const getFunFactText = () => {
    ...
    const facts =
      language === "zh" ? question.target.funFactsZh : question.target.funFacts;
    if (facts && facts[funFactIndex]) return facts[funFactIndex];
    return language === "zh"
      ? "这个国家非常独特！"
      : "This is a unique country!";
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] p-6 animate-fade-in">
        <div className="bg-[#e6ccb2] border-4 border-[#8b4513] p-8 max-w-md w-full text-center shadow-[8px_8px_0px_0px_rgba(0,0,0,0.5)] relative">
          ...
          <h2 className="text-3xl font-pixel-title text-[#8b4513] mb-4 leading-normal">
            {t.gameOver}
          </h2>
          <p className="text-[#5c4033] mb-8 text-2xl font-['VT323']">
            {t.youScored}{" "}
            <span className="text-[#d32f2f] font-bold text-3xl">{score}</span>{" "}
            {t.outOf} <span className="text-3xl">{QUESTIONS_PER_ROUND}</span>
          </p>
  ...
  if (!question)
    return (
      <div className="text-center p-10 text-[#d2b48c] font-pixel-title text-xl">
        {t.loading}
      </div>
    );
      ...
      <div className="flex items-center justify-between mb-4 bg-[#5c4033] p-3 border-4 border-[#3e2723] shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-white font-['VT323'] text-2xl">
            {currentQuestionIndex + 1} / {QUESTIONS_PER_ROUND}
          </span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-xs font-bold tracking-wider text-[#d2b48c] uppercase font-pixel-title">
      ...
      <div className="bg-[#e6ccb2] border-4 border-[#8b4513] shadow-[8px_8px_0px_0px_rgba(0,0,0,0.5)] relative transition-all duration-300">
        ...
        <div className={`p-4 grid gap-3 bg-[#e6ccb2] ${gridClasses}`}>
          {question.options.map((option) => {
            ...
            const opacityClass =
              selectedOption && !isTarget && !isSelected
                ? "opacity-50"
                : "opacity-100";
            ...
            return (
              ...
              >
                ...
                <span
                  className={`flex-1 text-left ${isFlagMode ? "text-center" : "hidden"}`}
                >
          ...
          <div className="bg-[#3e2723] p-4 border-t-4 border-[#8b4513] text-[#d2b48c] animate-slide-up">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-3">
              <div className="text-center md:text-left">
                <h3
                  className={`text-xl font-bold font-pixel-title ${isCorrect ? "text-[#76c442]" : "text-[#e25c5c]"}`}
                >
                ...
                {!isCorrect && (
                  <p className="text-[#a1887f] text-lg mt-1 font-['VT323'] flex items-center justify-center md:justify-start gap-2">
                    {t.correctAnswerWas}
                    <span
                      className={`text-[#fff8e1] font-bold ${!isFlagMode ? "text-3xl emoji-flag" : ""}`}
                    >
                      {isFlagMode
                        ? getCountryName(question.target)
                        : question.target.emoji}
              ...
              <Button
                onClick={handleNext}
                variant="primary"
                className="w-full md:w-auto min-w-[140px] py-2"
              >
            ...
            {/* Fun Fact Section */}
            {funFactText && (
              <div className="mt-1 bg-[#2a233e] p-3 border-2 border-[#5c4033] relative">
